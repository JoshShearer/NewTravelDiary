...
describe("Comps_Map\MyGoogleMap", () => {
  ...
  // it("should render a '' on the Comps_Map\MyGoogleMap and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_Map\MyGoogleMap.rename(oneProduct);
  //   renderWithRematchStore(<Comps_Map\MyGoogleMap />, store);
  ...
  // it("should reset the Comps_Map\MyGoogleMap to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_Map\MyGoogleMap.rename(otherProduct);
  //   renderWithRematchStore(<Comps_Map\MyGoogleMap />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
