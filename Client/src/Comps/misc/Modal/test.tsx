...
describe("Comps_misc\Modal", () => {
  ...
  // it("should render a '' on the Comps_misc\Modal and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_misc\Modal.rename(oneProduct);
  //   renderWithRematchStore(<Comps_misc\Modal />, store);
  ...
  // it("should reset the Comps_misc\Modal to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_misc\Modal.rename(otherProduct);
  //   renderWithRematchStore(<Comps_misc\Modal />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
