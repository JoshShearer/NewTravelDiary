...
describe("Comps_layout\imageHandler", () => {
  ...
  // it("should render a '' on the Comps_layout\imageHandler and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout\imageHandler.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout\imageHandler />, store);
  ...
  // it("should reset the Comps_layout\imageHandler to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout\imageHandler.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout\imageHandler />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
