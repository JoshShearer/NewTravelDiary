...
describe("Comps_layout\logContainer", () => {
  ...
  // it("should render a '' on the Comps_layout\logContainer and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout\logContainer.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout\logContainer />, store);
  ...
  // it("should reset the Comps_layout\logContainer to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout\logContainer.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout\logContainer />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
