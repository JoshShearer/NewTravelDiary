...
describe("Comps_layout\locTable", () => {
  ...
  // it("should render a '' on the Comps_layout\locTable and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout\locTable.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout\locTable />, store);
  ...
  // it("should reset the Comps_layout\locTable to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout\locTable.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout\locTable />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
