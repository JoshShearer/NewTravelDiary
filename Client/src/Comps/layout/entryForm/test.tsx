...
describe("Comps_layout_entryForm", () => {
  ...
  // it("should render a '' on the Comps_layout\entryForm and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout\entryForm.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout\entryForm />, store);
  ...
  // it("should reset the Comps_layout\entryForm to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout\entryForm.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout\entryForm />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
