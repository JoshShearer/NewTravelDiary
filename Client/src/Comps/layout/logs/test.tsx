...
describe("Comps_layout\logs", () => {
  ...
  // it("should render a '' on the Comps_layout\logs and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout\logs.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout\logs />, store);
  ...
  // it("should reset the Comps_layout\logs to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout\logs.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout\logs />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
